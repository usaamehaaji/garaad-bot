...
function guessHeist(robberId, guessCode) {
    ...
    if (guessCode[state.revealIndex] !== state.revealedDigit) {
        ...
        if (state.attemptsLeft <= 0) {
            ...
            saveData();
            activeHeists.delete(robberId);
            return { failure: true, attemptsLeft: 0, penalty: 100, message: 'Qiyaastaada waa khalad xad dhaaf ah. $100 cash ayaa lagaa jaray.' };
        }
    ...
    if (guessCode !== victim.password) {
        ...
        if (state.attemptsLeft <= 0) {
            ...
            saveData();
            activeHeists.delete(robberId);
            return { failure: true, attemptsLeft: 0, penalty: 100, message: 'Qiyaastaada waa khalad. Waxaa lagaa jaray $100 cash.' };
        }
    ...
    robber.cash ??= Number.isFinite(robber.usdBalance) ? robber.usdBalance : 0;
    const stolenCash = Math.floor((victim.cash || 0) * stealPercent);
