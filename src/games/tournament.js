...
async function cmdAdminNext(message) {
    ...
    const cid   = message.channel.id;
    const state = activeTournament.get(cid);
    if (!state) return message.reply(`⚠️ Tartan channel-kan ma jiro. Ugu horreyn \`${PREFIX}tartan_bilow\`.`);
    ...
    return message.reply('⚠️ Hadda admin_next looma isticmaali karo — sug inta wareeggu dhammaado.');
}
