...
async function topBanksCmd(message) {
    const all = Object.values(getAllPublicBanks()).sort((a, b) => (b.balance || 0) - (a.balance || 0)).slice(0, 10);
    if (!all.length) return message.reply('📭 Wali bank la abuuri waayay.');
    const lines = all.map((b, i) => `**${i + 1}.** 🏛️ **${b.name}** — ${fmtBtc(b.balance)} · ⭐${b.reputation || 0}`);
    return message.reply({
        embeds: [new EmbedBuilder()
            .setTitle('🏆 Top Banks')
            .setColor('#f39c12')
